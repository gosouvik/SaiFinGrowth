...
namespace GatiCarRental.Module.Controllers
{
    ...
    public partial class InvoicePrintController : ViewController
    {
    ...
    ReportDataProvider.ReportObjectSpaceProvider.CreateObjectSpace(typeof(ReportDataV2));
            ...
            if (record.Company.IsGSTApplicable)
            {
                reportData =
                    objectSpace.FindObject<ReportDataV2>(
                    CriteriaOperator.Parse("[DisplayName] = 'Invoice Email'"));
            }
            else
            {
                reportData =
                    objectSpace.FindObject<ReportDataV2>(
                    CriteriaOperator.Parse("[DisplayName] = 'Invoice NonGST'"));
            }
            ...
            CriteriaOperator criteria = new BinaryOperator("Invoice.InvoiceNo", record.InvoiceNo); // Filter by Tags  
        ...
        private void EmailInvoiceAction_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            Invoice record = (Invoice)((DevExpress.ExpressApp.DetailView)this.ObjectSpace.Owner).CurrentObject;
            IObjectSpace objectSpace =
    ReportDataProvider.ReportObjectSpaceProvider.CreateObjectSpace(typeof(ReportDataV2));
