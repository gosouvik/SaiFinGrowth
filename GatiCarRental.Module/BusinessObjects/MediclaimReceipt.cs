...
namespace GatiCarRental.Module.BusinessObjects
{
    ...
    public class MediclaimReceipt : BaseObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public MediclaimReceipt(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            _MediclaimReceiptID = (Session.Evaluate<MediclaimReceipt>(CriteriaOperator.Parse("Max(MediclaimReceiptID)"), CriteriaOperator.Parse("")) == DBNull.Value ? 0 : Convert.ToInt32(Session.Evaluate<MediclaimReceipt>(CriteriaOperator.Parse("Max(MediclaimReceiptID)"), CriteriaOperator.Parse("")))) + 1;
            ...
            //if (bookOrdObj != null)
            //DocumentNo = "RC-" + (_MediclaimReceiptID).ToString();
        ...
        private void GetDocumentNumbering()
        {
            ...
            docnos.Criteria = CriteriaOperator.Parse("[DocumentType]=?", DocumentType.MoneyMediclaimReceipt);
            if (docnos.Count == 1)
            {
        ...
        [Persistent("MediclaimReceiptID")] // this line for read-only columns mapping
        private int _MediclaimReceiptID;
        ////[RuleRequiredField] // Validation for Required
        [RuleUniqueValue] // Validation for unique value
        [Browsable(false)]
        [PersistentAlias("_MediclaimReceiptID")] // This line for read-only column mapping
        public int MediclaimReceiptID
        {
            get { return _MediclaimReceiptID; }
        }
        ...
        private XPCollection<MediclaimPolicy> fAvailableLICPolies;
        [Browsable(false)] // Prohibits showing the AvailableAccessories collection separately
        public XPCollection<MediclaimPolicy> AvailableLICPolies
        {
            get
            {
                if (fAvailableLICPolies == null)
                {
                    ...
                    fAvailableLICPolies = new XPCollection<MediclaimPolicy>(Session);
        ...
        public MediclaimPolicy MediclaimPolicies
        {
            get { return fMediclaimPolicies; }
            set
            {
                SetPropertyValue(nameof(MediclaimPolicy), ref fMediclaimPolicies, value);
                if (fMediclaimPolicies != null)
                {
                    if (this.fMediclaimPolicies.Customer != null)
                    {
                        this.Customer = this.fMediclaimPolicies.Customer;
                    }
                }
            }
        }
        MediclaimPolicy fMediclaimPolicies;
        ...
        public XPCollection<MediclaimReceiptFileData> Attachments
        {
            get { return GetCollection<MediclaimReceiptFileData>(nameof(Attachments)); }
        }
