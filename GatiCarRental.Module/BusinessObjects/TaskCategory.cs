...
namespace GatiCarRental.Module.BusinessObjects
{
    ...
    public class TaskCategory : BaseObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public TaskCategory(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            _TaskCategoryID = (Session.Evaluate<TaskCategory>(CriteriaOperator.Parse("Max(TaskCategoryID)"), CriteriaOperator.Parse("")) == DBNull.Value ? 0 : Convert.ToInt32(Session.Evaluate<TaskCategory>(CriteriaOperator.Parse("Max(TaskCategoryID)"), CriteriaOperator.Parse("")))) + 1;
        ...
        [Persistent("TaskCategoryID")] // this line for read-only columns mapping
        private int _TaskCategoryID;
        ////[RuleRequiredField] // Validation for Required
        [RuleUniqueValue] // Validation for unique value
        [Browsable(false)]
        [PersistentAlias("_TaskCategoryID")] // This line for read-only column mapping
