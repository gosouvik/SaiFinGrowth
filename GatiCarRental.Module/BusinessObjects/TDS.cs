...
namespace GatiCarRental.Module.BusinessObjects
{
    ...
    public class TDS : BaseObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public TDS(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            _TDSID = (Session.Evaluate<TDS>(CriteriaOperator.Parse("Max(TDSID)"), CriteriaOperator.Parse("")) == DBNull.Value ? 0 : Convert.ToInt32(Session.Evaluate<TDS>(CriteriaOperator.Parse("Max(TDSID)"), CriteriaOperator.Parse("")))) + 1;
        ...
        private void GetDocumentNumbering()
        {
            ...
            docnos.Criteria = CriteriaOperator.Parse("[DocumentType]=?", DocumentType.TDS);
            if (docnos.Count == 1)
            {
        ...
        [Persistent("TDSID")] // this line for read-only columns mapping
        private int _TDSID;
        ////[RuleRequiredField] // Validation for Required
        [RuleUniqueValue] // Validation for unique value
        [Browsable(false)]
        [PersistentAlias("_TDSID")] // This line for read-only column mapping
        public int TDSID
        {
            get { return _TDSID; }
        }
